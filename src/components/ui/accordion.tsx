...
>(({className, children, ...props}, ref) => (
    <AccordionPrimitive.Header className="flex">
        <AccordionPrimitive.Trigger
            ref={ref}
            className={cn(
                "flex flex-1 items-center justify-between py-4 px-10 text-sm font-medium transition-all group text-left [&[data-state=open]>svg]:rotate-45 " +
                "text-card-foreground text-xl font-light",
                className
            )}
            {...props}
        >
